<script lang="ts">
  ...
  function readCachedSellers(): `0x${string}`[] | null {
    ...
    try {
      ...
      const list = Array.isArray(parsed?.sellers)
        ? parsed.sellers.filter(
            (v) => typeof v === 'string' && v.startsWith('0x')
          )
        : [];
      return Array.from(
        new Set(list.map((v) => v.toLowerCase()))
      ) as `0x${string}`[];
    } catch {
      return null;
    }
  ...
  $effect(() => {
    ...
    if (!ops || ops.length === 0) {
      selectedOperator = gnosisConfig.production
        .marketOperator as `0x${string}`;
      return;
    }
  ...
  async function loadSellers(): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Failed to load sellers (${res.status}): ${text || res.statusText}`
        );
      }
      ...
      list =
        (Array.isArray(body)
          ? body
          : body && Array.isArray((body as SellersResponse).sellers)
            ? (body as SellersResponse).sellers
            : []) ?? [];
    } catch (err) {
      list = [
        {
    ...
    try {
      ...
      const op = (selectedOperator ??
        (gnosisConfig.production
          .marketOperator as `0x${string}`)) as `0x${string}`;
  ...
  const actions: Action[] = [
    ...
    },
    {
      ...
      label: 'Orders',
      variant: 'ghost',
      onClick: () => goto('/settings?tab=orders'),
    },
    {
      id: 'settings-sales',
      label: 'Sales',
      variant: 'ghost',
      onClick: () => goto('/settings?tab=sales'),
    },
  ];
</script>
...
>
  ...
  {#if loading && products.length === 0}
    <section class="bg-base-100 border border-base-300 rounded-3xl p-4">
        ...
        {#each Array(6) as _, i}
          ...
          >
            ...
            <div class="p-3 flex flex-col gap-1">
              ...
              <div
                class="min-h-[2.25rem] flex items-center justify-between mt-2"
              >
