<script lang="ts">
  ...
  const virtualList = getContext<VirtualListController | undefined>(
    VIRTUAL_LIST_CONTEXT_KEY
  );
  ...
  function onRowKeydown(event: KeyboardEvent): void {
    ...
    const virtualIndexAttr = current?.closest<HTMLElement>(
      '[data-virtual-index]'
...
>
  <RowFrame
    ...
    dense={true}
    noLeading={true}
    style="min-height: var(--transaction-row-height, 76px);"
  >
