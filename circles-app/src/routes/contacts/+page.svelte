<script lang="ts">
  ...
  function getInitialPageCount(): number {
    if (!browser) return 1;
    const raw = Number(
      window.sessionStorage.getItem(CONTACTS_VISIBLE_COUNT_KEY) ?? '0'
    );
  ...
  async function preloadProfileCores(addresses: string[]) {
    ...
    const missing = normalized.filter(
      (addr) => !cacheSnapshot.has(addr) && !inflightProfileRequests.has(addr)
    );
        ...
        .map(([address, contact]) => ({
          ...
          contact: {
            ...
            contactProfile:
              ...
              $profileCache.get(address.toLowerCase()),
          },
        }));
    }
  );
  ...
  const searchedAll = derived(
    [filteredAll, searchQuery],
    ([$filteredAll, $searchQuery]) => {
  ...
  const contactsPaginatedWithEnd = derived(
    [contactsPaginated, contacts],
    ([$paginated, $contacts]) => {
  ...
  const actions: Action[] = $derived([
    ...
    },
    {
      ...
      label: 'Bookmarks',
      iconNode: Star,
      onClick: () => goto('/settings?tab=bookmarks'),
      variant: 'ghost',
    },
    {
      id: 'export',
  ...
  {#snippet collapsedLeft()}
    <div class="truncate flex items-center gap-2">
      ...
      <span class="text-sm text-base-content/60"
        >{$filteredAll.length} {countLabel}</span
      >
    </div>
  {/snippet}

  {#snippet collapsedInlineActions()}
    <ActionButtonBar actions={getLeadingActions(actions, 2)} iconOnly={true} />
  {/snippet}
    ...
    <div id={FILTER_PANEL_ID} class="mt-3 mb-3 space-y-3">
      {#if !avatarState.isGroup}
        ...
        <p class="text-xs text-base-content/70">
          Trust decides which Circles you accept — and whether payments can
          route through your connections.
        </p>
      {/if}
    </div>
  {/if}
