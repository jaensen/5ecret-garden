...
<div class="space-y-6">
  {#snippet profileActions()}
    {#if !avatarState.isGroup}
      <button
        class="btn btn-muted-outline btn-sm"
        onclick={() => {
          openSendFlowPopup({
  ...
  {:else}
    <div class="flex items-center justify-end gap-2 flex-wrap -mt-1 mb-1 sm:mb-2">
      {@render profileActions()}
    </div>
  {/if}
  ...
  <ProfileHeroCard
    ...
    {relationText}
    {relationOverlayUrl}
    hasTrustRow={!!trustRow}
    relationIsPositive={trustRow?.relation === 'trusts' || trustRow?.relation === 'trustedBy' || trustRow?.relation === 'mutuallyTrusts'}
    {isBookmarked}
    {showBookmarkEditor}
    {bookmarkFolders}
