<script lang="ts">
  ...
  function productKeyForCatalogItem(
  ...
  function resolvePriceAmount(item: AggregatedCatalogItem): number | null {
    const offer: any =
      (item as any)?.offer ??
      (item as any)?.offers ??
      (item as any)?.product?.offer ??
      (item as any)?.product?.offers;
  ...
  function productImageUrl(item: AggregatedCatalogItem): string | null {
    ...
    if (Array.isArray(img)) {
      const first = img.find((x) => typeof x === 'string') as
        | string
        | undefined;
      return first ?? null;
    }
  ...
  async function loadCatalog(): Promise<void> {
    ...
    try {
      ...
      const filtered = items.filter(
        (p) => (p.seller ?? '').toLowerCase() === seller.toLowerCase()
      );
  ...
  function resolveProductTitle(item: AggregatedCatalogItem): string {
    ...
    const sku =
      normalizeSku((item as any)?.product?.sku ?? '') ??
      String((item as any)?.product?.sku ?? '').trim();
          ...
          {#snippet leading()}
            {#if imgUrl}
              <img
                src={imgUrl}
                alt=""
                class="w-10 h-10 rounded-md object-cover bg-base-200"
              />
            {:else}
