<script lang="ts">
  ...
  import {
    ...
    useResolvedProducts,
  } from '$lib/areas/market/flows/checkout/useResolvedProducts';
  ...
  async function handleQuantityChange(
    ...
    if (!seller || !sku) {
      ...
      console.warn('[cart] line has no seller/sku; cannot update quantity', {
        line,
        itemIdx,
      });
      return;
  ...
  const checkoutAction = useAsyncAction(
    ...
    },
    {
      onError: (e) => {
        ...
        console.warn(
          '[cart] validate before checkout failed; opening details step',
          e
        );
        openStep({
          title: 'Additional details',
  ...
  const isCheckedOut = $derived.by(
    () => $cartState.basket?.status === 'CheckedOut'
  );
</script>
...
>
  ...
  {#if !$cartState.basket || !$cartState.basket.items || $cartState.basket.items.length === 0}
    <div class="p-4 text-sm opacity-70">Your basket is empty.</div>
  {:else}
    <div class="space-y-2">
      <OrderLineTable
        lines={cartLines}
        ...
        getLineUnitPrice={(line) => ({
          amount:
            typeof line?.offerSnapshot?.price === 'number'
              ? line.offerSnapshot.price
              : null,
          code:
            typeof line?.offerSnapshot?.priceCurrency === 'string'
              ? line.offerSnapshot.priceCurrency
              : null,
        })}
        getLineTotal={(line) => ({
          amount:
    ...
    {#if $cartState.basket?.status === 'CheckedOut' && $cartState.lastCheckout?.paymentReference}
      <div class="mt-3 alert alert-success text-xs">
        ...
        <code class="ml-1 break-all"
          >{$cartState.lastCheckout.paymentReference}</code
        >
      </div>
    {/if}
    ...
    <StepActionBar>
      {#snippet primary()}
        <button
          ...
          disabled={checkoutAction.loading ||
            $cartState.loading ||
            !$cartState.basket ||
            !$cartState.basket.items ||
            $cartState.basket.items.length === 0 ||
