<script lang="ts">
  ...
  $effect(() => {
    selectedGateway = (context.draft?.paymentGateway ?? '') as string;
  });
  ...
  const requiredOk = $derived.by(() => {
    ...
    const hasOffer =
      (d?.price ?? 0) > 0 && /^[A-Z]{3}$/.test(d?.priceCurrency ?? '');
  ...
  async function publish(): Promise<void> {
    ...
    const hasLegacyImage =
      typeof draft.image === 'string' && draft.image.length > 0;
    const productImages = hasImagesArray
      ? draft.images
      : hasLegacyImage
        ? [draft.image]
        : undefined;
    ...
    await runTask({
      ...
      promise: (async () => {
        ...
        if (Array.isArray(productImages) && productImages.length > 0) {
          if (!media) {
            throw new Error(
              'Media pinning not available (missing pinApiBase).'
            );
          }
        ...
        context.result = await client.appendOffer({
          ...
          offer: {
            ...
            requiredSlots:
              Array.isArray(draft.requiredSlots) &&
              draft.requiredSlots.length > 0
                ? draft.requiredSlots
                    .map((s: unknown) =>
...
>
  {#if !requiredOk}
    <StepAlert
      variant="warning"
      className="mb-2"
      message="Draft has missing or invalid fields."
    />
  {/if}
  ...
  <div class="space-y-2">
    <StepSection
      title="Review"
      subtitle="Check core product and pricing details."
    >
      <div class="space-y-3">
        <StepReviewRow
          label="Product"
          value={context.draft?.name ?? '—'}
          onChange={editProduct}
          changeLabel="Edit"
        />
        <StepReviewRow
          label="Pricing"
          value={`${context.draft?.price ?? '—'} ${context.draft?.priceCurrency ?? ''}`.trim()}
        ...
        <div slot="meta" class="text-sm space-y-2">
          ...
          {#if selectedGateway}
            <div class="flex items-center gap-2">
              <Avatar
                address={asAddress(selectedGateway)}
                view="small_no_text"
                clickable={false}
              />
          ...
          {:else}
            <span class="opacity-70 text-sm"
              >No payment gateway selected. Go back to Pricing to select one.</span
            >
          {/if}
          <div>
            <strong>Price:</strong>
            {context.draft?.price}
            {context.draft?.priceCurrency}
          </div>
        </div>
      </ProductPreviewCard>
    ...
    <StepSection
      ...
      subtitle="Confirm delivery method and required checkout info."
    >
      ...
      {:else}
        <div class="text-sm text-base-content/70">
          No checkout requirements specified.
        </div>
      {/if}
    </StepSection>
