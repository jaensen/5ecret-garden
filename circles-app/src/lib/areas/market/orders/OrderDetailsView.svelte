<script lang="ts">
  ...
  $effect(() => {
    ...
    const history = Array.isArray(statusEvents)
      ? statusEvents
          .filter(
            (ev) =>
              ev &&
              typeof ev.newStatus === 'string' &&
              typeof ev.changedAt === 'string'
          )
          .slice()
          .sort((a, b) =>
    ...
    if (history.length > 0) {
      ...
      const initialStatus =
        (first.oldStatus && String(first.oldStatus)) || null;
  ...
  function isVoucherPayload(payload: any): boolean {
    return safeParse(
      ...
      () => {
        ...
        const hasType =
          t.includes('voucher') || t.includes('codedispenserresult');
        const hasSingle =
  ...
  function isSchemaType(payload: any, endsWith: string): boolean {
    ...
    return (
      lower.endsWith('/' + end) || lower.endsWith('#' + end) || lower === end
    );
  }
  ...
  function evmFromEip155(
    ...
    if (parts.length >= 3) {
      ...
      if (/^0x[a-fA-F0-9]{40}$/.test(addr))
        return addr as unknown as EvmAddress;
    }
    return undefined;
  }
  ...
  function unitPrice(i: number): {
    ...
    return safeParse(
      ...
      () => {
        const offer = (snapshot as any)?.acceptedOffer?.[i];
        const amt =
          typeof offer?.price === 'number' ? (offer.price as number) : null;
  ...
  async function resolveLine(i: number) {
    try {
      ...
      const direct =
        typeof lineAt(i)?.imageUrl === 'string'
          ? lineAt(i).imageUrl.trim()
          : '';
      if (direct) {
        resolved[i] = {
          name: lineAt(i)?.orderedItem?.name ?? null,
          imageUrl: direct,
        };
        return;
      }
      ...
      const prod = await catalog.fetchProductForSellerAndSku(
        String(evm),
        String(sku)
      );
      if (!prod) {
  ...
  $effect(() => {
    ...
    sellerGroups = Array.from(map.entries()).map(([key, indices]) => {
      ...
      return {
        sellerId,
        evm: evmFromEip155(sellerId ?? undefined),
        indices,
      } as SellerGroup;
    });
...
{#if snapshot}
  <section
    class="w-full bg-base-100 border rounded-3xl shadow-sm overflow-hidden"
  >
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2">
          <span
            ...
            title={snapshot.orderNumber}>{snapshot.orderNumber}</span
          >
        </div>
    ...
    <div
      class="px-4 md:px-5 pb-4 md:pb-5 grid grid-cols-1 md:grid-cols-2 gap-4"
    >
      <div class="space-y-1">
        ...
        {:else}
          <div class="font-mono text-sm break-all">
            {partyId(getSchemaId(snapshot.customer))}
          </div>
        {/if}
      </div>
      <div class="space-y-1">
        ...
        {:else}
          <div class="font-mono text-sm break-all">
            {partyId(getSchemaId(snapshot.broker))}
          </div>
        {/if}
      </div>
      ...
      {#if snapshot.shippingAddress}
        <div class="space-y-1">
          ...
          <div class="text-sm leading-snug">
            ...
            <br />{snapshot.shippingAddress.postalCode}
            {snapshot.shippingAddress.addressLocality}
      ...
      {#if snapshot.billingAddress}
        <div class="space-y-1">
          ...
          <div class="text-sm leading-snug">
            ...
            <br />{snapshot.billingAddress.postalCode}
            {snapshot.billingAddress.addressLocality}
    ...
    <div
      class="px-4 md:px-5 py-3 border-t text-xs uppercase tracking-wide opacity-60"
    >
      Items
    </div>
    <div class="px-4 md:px-5 pb-4 md:pb-5 flex flex-col gap-3">
      {#each sellerGroups as grp, gi (gi)}
        ...
        <div class="border rounded-lg overflow-hidden bg-base-100/80 shadow-sm">
          <div
            class="px-3 py-2 border-b bg-base-200/60 flex items-center justify-between"
          >
            <div class="min-w-0">
              {#if grp.evm}
                <Avatar
              ...
              {:else}
                ...
                <div class="font-mono text-sm break-all">
                  {grp.sellerId ?? 'Unknown'}
                </div>
              {/if}
            </div>
            <div class="shrink-0 text-[11px] opacity-60">
              {grp.indices.length} item{grp.indices.length === 1 ? '' : 's'}
            </div>
          </div>
          ...
          <div class="divide-y divide-base-200">
            {#each grp.indices as i}
              <div
                class="px-3 py-2 bg-base-200/20 cursor-pointer hover:bg-base-200/40 focus:outline-none focus:ring-2 focus:ring-primary/40"
                role="button"
                tabindex="0"
                onclick={() => goToOffer(i)}
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  ...
                  <div class="flex items-center gap-3 min-w-0">
                    <div
                      class="w-14 h-14 rounded bg-base-200 overflow-hidden shrink-0 flex items-center justify-center"
                    >
                      {#if resolved[i]?.imageUrl}
                        <img
                          src={resolved[i]?.imageUrl || ''}
                          alt={resolved[i]?.name ??
                            lineAt(i)?.orderedItem?.sku ??
                            'Product image'}
                    ...
                    <div class="min-w-0">
                      <div class="font-semibold truncate">
                        {resolved[i]?.name ||
                          lineAt(i)?.orderedItem?.name ||
                          lineAt(i)?.orderedItem?.sku ||
                          'Item'}
                      </div>
                      <div class="text-xs opacity-70 mt-0.5">
                        Qty: {lineAt(i)?.orderQuantity ?? 1}
                      </div>
                      {#if lineAt(i)?.productCid}
                        <div class="font-mono text-[11px] opacity-70 break-all">
                          {lineAt(i)?.productCid}
                        </div>
                      {/if}
                    </div>
                  ...
                  <div class="text-right shrink-0">
                    {#if unitPrice(i).amount != null}
                      <div class="text-xs opacity-70">
                        {formatCurrency(unitPrice(i).amount, unitPrice(i).code)} each
                      </div>
                    {/if}
    ...
    <div
      class="px-4 md:px-5 pt-3 border-t text-xs uppercase tracking-wide opacity-60"
    >
      Status history
    </div>
      ...
      {:else}
        {#each timeline as evt, idx}
          <div class="flex items-start gap-2">
            <div
              class="mt-1 w-2 h-2 rounded-full {idx === timeline.length - 1
                ? 'bg-primary'
                : 'bg-base-300'}"
            ></div>
            ...
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                {#if !isMessagePayload(item.payload)}
                  <span
                    class={`badge badge-xs ${badgeForOutbox(item.payload)}`}
                  >
...
{#snippet OutboxPayloadView({ payload })}
    ...
    <div class="text-xs flex items-start gap-2">
      ...
      <ul class="m-0 p-0 list-none space-y-1.5">
        {#each voucherCodes(payload) as c}
          <li class="max-w-full">
            <code
              class="font-mono text-[11px] break-all inline-block max-w-full px-1.5 py-1 bg-base-200/80 border border-base-300 rounded"
              >{c}</code
            >
          </li>
        {/each}
  ...
  {:else if isMessagePayload(payload)}
    <div class="flex flex-col gap-1.5">
      ...
      {#if payload.text}
        <div class="text-sm leading-relaxed whitespace-pre-line opacity-90">
          {payload.text}
        </div>
      {/if}
    </div>
  {:else}
    <pre
      class="text-[11px] bg-base-200/80 rounded-md p-2 overflow-auto">{JSON.stringify(
        payload,
        null,
        2
      )}</pre>
