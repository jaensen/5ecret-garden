<script lang="ts">
  ...
  async function createGroup() {
    ...
    const profileNameOk: boolean =
      profileNameLength > 0 && profileNameLength <= PROFILE_NAME_MAX_LENGTH;
    ...
    if (!profileNameOk || !symbolOk || !onChainOk) {
      throw new Error(
        'Invalid profile name (required, max 36 chars), symbol, or on-chain name'
      );
    }
    ...
    await runTask({
      ...
      promise: (async () => {
        ...
        if (!tx) {
          throw new Error('Failed to submit createBaseGroup transaction');
        }
...
>
  ...
  <div class="mt-4 space-y-1">
    ...
    <div>
      <span class="text-base-content/70 mr-1">On-chain name:</span>{onChainName}
    </div>
  </div>
  ...
  <AdvancedDetails title="Advanced group details" subtitle="On-chain settings">
    {#if !fastLane}
      ...
      <Avatar
        ...
        view="horizontal"
        clickable={false}
        bottomInfo={ctx.service}
        showTypeInfo={true}
      />
      <div class="text-xs text-base-content/60">Fee collection</div>
      <Avatar
        address={ctx.feeCollection}
        view="horizontal"
        clickable={false}
        bottomInfo={ctx.feeCollection}
        showTypeInfo={true}
      />
      <div>
    ...
    {:else}
      <div class="text-sm text-base-content/70">
        Fast mode uses default service + fee collection.
      </div>
    {/if}
  </AdvancedDetails>
