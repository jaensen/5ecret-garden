<script lang="ts">
  ...
  $effect(() => {
    ...
    const shouldDefaultFee: boolean =
      ctx.feeCollection === zero && !!$wallet?.address;
    if (shouldDefaultFee) {
  ...
  const profileNameValid: boolean = $derived(
    profileNameTrimmed.length > 0 &&
      profileNameTrimmed.length <= PROFILE_NAME_MAX_LENGTH
  );
  const symbolHasInput: boolean = $derived(
    ctx.profile.symbol.trim().length > 0
  );
  const symbolValidNow: boolean = $derived(isValidSymbol(ctx.profile.symbol));
  const showSymbolInvalid: boolean = $derived(
    symbolHasInput && !symbolValidNow
  );
  ...
  const onChainNameValid = $derived(
    onChainNameHasInput && isValidOnChainName(trimmedOnChainName)
  );
  const canContinue: boolean = $derived(
    profileNameValid && symbolValidNow && onChainNameValid
  );
    ...
    <label class="form-control">
      <div class="label">
        <span class="label-text"
          >Symbol <Tooltip content="Short currency symbol (e.g., CRC)." /></span
        >
      </div>
      <input
        required
      ...
      <div class="h-5 text-xs text-error pt-1">
        {#if showSymbolInvalid}Invalid symbol{/if}
      </div>
    </label>
