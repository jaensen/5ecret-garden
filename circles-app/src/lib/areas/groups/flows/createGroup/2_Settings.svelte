<script lang="ts">
  ...
  function parseAddresses(csv: string): {
    valid: `0x${string}`[];
    invalid: string[];
  } {
    const parts = csv
  ...
  const serviceLooksValid: boolean = $derived(
    typeof ctx.service === 'string' &&
      ctx.service.length === 42 &&
      ethers.isAddress(ctx.service)
  );
  const feeLooksValid: boolean = $derived(
    typeof ctx.feeCollection === 'string' &&
      ctx.feeCollection.length === 42 &&
      ethers.isAddress(ctx.feeCollection)
  );
  const advancedValid: boolean = $derived(
    serviceLooksValid && feeLooksValid && parsed.invalid.length === 0
  );
...
>
  ...
  <div class="mt-4 space-y-3">
    <label
      class="flex items-start gap-3 p-3 rounded-3xl border border-base-300 cursor-pointer"
    >
      <input
        type="radio"
      ...
      <div>
        ...
        <div class="text-xs text-base-content/70">
          Use your wallet for fee collection, no service contract, no initial
          conditions.
        </div>
      </div>
    </label>
    ...
    >
      ...
      <div>
        ...
        <div class="text-xs text-base-content/70">
          Set custom service and fee collection addresses and initial
          conditions.
        </div>
      </div>
    </label>
  ...
  {#if mode === 'advanced'}
    ...
    <label class="form-control mt-4">
      ...
      />
      <div
        class="h-5 text-xs pt-1"
        class:text-error={!serviceLooksValid}
        class:text-success={serviceLooksValid}
      >
        {#if serviceLooksValid}Looks good{/if}{#if !serviceLooksValid}Invalid
          address{/if}
      </div>
    </label>
    ...
    <label class="form-control">
      ...
      <div
        class="h-5 text-xs pt-1"
        class:text-error={!feeLooksValid}
        class:text-success={feeLooksValid}
      >
    ...
    <label class="form-control">
      <div class="label">
        <span class="label-text">
          Initial conditions
          <Tooltip
            content="Comma-separated list of addresses added at creation time."
          />
        </span>
      </div>
      <input
