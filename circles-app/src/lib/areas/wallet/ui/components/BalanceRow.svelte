<script lang="ts">
  ...
  function focusBalancesSearchInput(anchor?: HTMLElement | null): void {
    const scope = anchor?.closest<HTMLElement>(
      '[data-balances-list-scope], [data-select-asset-list-scope]'
    );
    const input =
      scope?.querySelector<HTMLInputElement>(
        '[data-balances-search-input], [data-select-asset-search-input]'
      ) ??
  ...
  const listNavigator = createKeyboardListNavigator({
    getRows: (anchor) => {
      ...
      return Array.from(
        (scope ?? document).querySelectorAll<HTMLElement>('[data-balance-row]')
      );
    },
  ...
  async function handleReadHubBalance(): Promise<void> {
    ...
    try {
      ...
      const message =
        error?.message ?? 'Failed to query balance from Circles V2 hub.';
      await openInfoPopup({
      ...
      <div class="flex items-center gap-3 md:gap-4 shrink-0">
        <div class="text-right tabular-nums">
          <div class="font-medium">
            {formatCompactCurrency(item.circles, 'CRC')}
          </div>
        ...
        {#if !avatarState.isGroup}
          ...
          >
            ...
            <ul
              class="dropdown-content menu menu-sm bg-base-100 rounded-box shadow z-[200] w-56 p-2"
              onclick={(e) => e.stopPropagation()}
            >
