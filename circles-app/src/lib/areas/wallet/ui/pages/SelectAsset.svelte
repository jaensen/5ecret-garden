...
<script lang="ts">
  ...
  import SelectableBalanceRow, {
    type SelectableBalanceRowItem,
  } from '$lib/areas/wallet/ui/components/SelectableBalanceRow.svelte';
  ...
  const emptySelectable = readable<{
    data: SelectableBalanceRowItem[];
    next: () => Promise<boolean>;
    ended: boolean;
  }>({
    data: [],
...
<ListShell
  ...
  isNoMatches={$balances?.data?.length > 0 &&
    $selectableBalances.data.length === 0}
