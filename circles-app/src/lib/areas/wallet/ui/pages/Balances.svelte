<script lang="ts">
  ...
  onMount(() => {
    ...
    const alreadyDismissed =
      localStorage.getItem(BALANCES_HELP_DISMISSED_KEY) === '1';
...
{#snippet balancesToolbarActions()}
  ...
  <HelpPopover
    bind:open={showBalancesHelp}
    title="Why so many Circles?"
    lines={WHY_MANY_CIRCLES_LINES}
    align="end"
    side="top"
    openOnHover={true}
    dismissible={true}
    onDismiss={dismissBalancesHelp}
    buttonClass="btn btn-ghost btn-sm btn-circle border border-base-300/70 bg-base-100 hover:bg-base-200"
