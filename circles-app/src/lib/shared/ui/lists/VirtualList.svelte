<script lang="ts">
    ...
    for (let i = start; i < end; i += 1) {
      ...
      if (i < loadedItems.length) {
        ...
        out.push({
          ...
          index: i,
          top,
          placeholderIndex: i - loadedItems.length,
        });
      }
    }
  ...
  function computePlaceholderPageSize(): number {
    ...
    const hasExpectedPageSize =
      typeof expectedPageSize === 'number' && expectedPageSize > 0;
  ...
  function clearPlaceholdersAfterLoad(
    addedCount: number,
    storeEnded: boolean
  ): void {
    if (storeEnded) {
  ...
  function isHiddenByAncestor(el: HTMLElement | undefined): boolean {
    if (!el) return true;
  ...
  function isListVisible(): boolean {
    if (!listEl || typeof window === 'undefined') return false;
    if (isHiddenByAncestor(listEl)) return false;
    return listEl.getClientRects().length > 0;
  }
  ...
  function scrollToIndex(index: number): void {
    ...
    if (!root) {
      ...
      const nextScrollTop = computeJumpScrollTop(
        viewportTop,
        viewportHeight,
        rowTop,
        rowBottom
  ...
  function focusIndex(index: number): void {
    ...
    const tryFocus = () => {
      ...
      const rowContainer = listEl.querySelector<HTMLElement>(
        `[data-virtual-index="${clamped}"]`
      );
      const focusable =
  ...
  function updateRange(): void {
    ...
    const rootRect = root
      ...
      : ({ top: 0, bottom: window.innerHeight } as Pick<
          DOMRect,
          'top' | 'bottom'
        >);
  ...
  function maybePrefetchNext(): void {
    ...
    const runway = Math.max(
      3,
      Math.floor(ensurePlaceholderPageSize() * eagerLoadMultiplier)
    );
  ...
  function setupListeners(): () => void {
    ...
    const panel = listEl.closest<HTMLElement>('[role="tabpanel"]');
...
<div
  class="generic-list virtual-list ui-scrollbar-thin w-full py-2"
  role="list"
  bind:this={listEl}
>
  <div
  ...
  >
    ...
    {:else if hasError}
      <span class="text-error"
        >{String(displayError ?? 'Error loading items')}</span
      >
