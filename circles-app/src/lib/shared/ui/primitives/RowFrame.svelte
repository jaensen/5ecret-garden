<script lang="ts">
  ...
  function handleKeydown(e: KeyboardEvent): void {
    onkeydown?.(
      e as KeyboardEvent & { currentTarget: EventTarget & HTMLDivElement }
    );
...
<style>
  ...
  .ui-row[data-clickable] {
    cursor: pointer;
  }
  .ui-row[data-clickable]:hover {
    background: var(--row-bg-hover);
    border-color: var(--row-border-hover);
  }
  .ui-row[data-clickable]:active {
    transform: translateY(0.5px);
  }
  .ui-row:focus-visible {
    outline: none;
    box-shadow: 0 0 0 3px var(--row-focus-ring);
  }
